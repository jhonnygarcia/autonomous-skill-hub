...
def test_the_language_directive_travels_in_the_prompt(client, monkeypatch):
    ...
    assert "in Spanish" not in prompt
...
def test_the_language_directive_reaches_every_fan_out_child(client, monkeypatch, tmp_path):
    ...
    client.put("/idioma", json={"idioma": "en"})
    _use_fake_claude(monkeypatch, stamp="ok — survey.md")
    _write_brief(tmp_path, 61, routing="SONDEAR: front, backend")
    calls = _spy_all_argv(monkeypatch)
    tid = client.post("/tickets", json={"ado_id": 61, "project": "Demo"}).json()["id"]
    client.post(f"/tickets/{tid}/run", json={"phase": "survey"})
    child_prompts = [c[c.index("-p") + 1] for c in calls]
    assert child_prompts and all("in English" in p for p in child_prompts)


def test_the_resume_prompt_carries_the_current_language(client, monkeypatch):
    ...
    _use_fake_claude(monkeypatch)
    tid = client.post("/tickets", json={"ado_id": 62, "project": "Demo"}).json()["id"]
    client.post(f"/tickets/{tid}/run", json={})          # first run, Spanish
    client.put("/idioma", json={"idioma": "en"})
    cap = _spy_argv(monkeypatch)
    client.post(f"/tickets/{tid}/run", json={"resume": True})
